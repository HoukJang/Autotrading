...
_REGIME_COMPAT: dict[tuple[str, str], float] = {
    # Trend-following strategies are highly compatible with trend regime
    ("adx_pullback", "long"): 0.95,
    ("regime_momentum", "long"): 0.95,
    ("regime_dual", "long"): 0.90,
    # Mean-reversion strategies work in ranging/low-volatility regimes
    ("rsi_mean_reversion", "long"): 0.80,
    ("rsi_mean_reversion", "short"): 0.80,
    ("bb_squeeze", "long"): 0.75,
    ("bb_squeeze", "short"): 0.75,
    # Short strategies are compatible with high-vol or ranging regimes
    ("overbought_short", "short"): 0.70,
}
...
def _regime_compatibility(scan_result: ScanResult) -> float:
    ...
    # Boost trend strategies when ADX is strong (> 25)
    if scan_result.direction in ("long",) and scan_result.strategy in (
        "adx_pullback",
        "regime_momentum",
        "regime_dual",
    ):
...
class SignalRanker:
    ...
    def rank(
        ...
        # Step 4: Final sort by composite score, tie-break on signal_strength,
        # then symbol for determinism.
        candidates.sort(
            key=lambda c: (-c.composite_score, -c.signal_strength, c.symbol)
        )

        # Step 5: Select top-N and assign ranks
